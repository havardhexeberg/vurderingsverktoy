...
const navigation = [
  { name: "Min oversikt", href: "/elev", icon: LayoutDashboard },
  { name: "Kompetansemål", href: "/elev/kompetanse", icon: Target },
  { name: "Vurderinger", href: "/elev/vurderinger", icon: BookOpen },
]
    ...
    <>
      <div className="flex h-16 items-center justify-between px-4">
        <Link href="/elev" className="flex items-center gap-2">
          ...
          <span className="text-lg font-semibold">Elevportal</span>
        </Link>
        <Button
          variant="ghost"
          size="icon"
      ...
      <nav className="flex-1 space-y-1 px-2 py-4">
        {navigation.map((item) => {
          const isActive = pathname === item.href ||
            (item.href !== "/elev" && pathname.startsWith(item.href))
          return (
            <Link
              key={item.name}
              href={item.href}
      ...
      <div className="p-4">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-cyan-600 truncate">Elev</p>
          </div>
        </div>
        <Button
          variant="outline"
